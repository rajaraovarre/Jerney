...
locals {
  azs = slice(data.aws_availability_zones.available.names, 0, 3)    #syntax: slice(list, start, end)
}
...
module "vpc" {
  ...
  azs             = local.azs
  private_subnets = [for k, v in local.azs : cidrsubnet(var.vpc_cidr, 4, k)]  #syntax: [for index, value in list : expression]
